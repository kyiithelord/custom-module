...
class PurchaseOrderReport(models.TransientModel):
    ...
    def get_xlsx(self, response):
        ...
        y_offset = 0
        row_no = 0
        sheet.merge_range(y_offset, 0, y_offset, 10, 'Purchase Order Report', title_style)
        y_offset += 2
        ...
        y_offset += 1
        sheet.merge_range(y_offset,8,y_offset,9,_('Amount Total'),header_style_gray)
        sheet.write(y_offset,10,amount_total,serial_no_style)
        sheet.write(y_offset,5,_('Total Qty'),header_style_gray)
        sheet.write(y_offset,6,quantity_total,serial_no_style)
        workbook.close()
        excel.seek(0)
